...
// and MCP surfaces cannot describe different behaviour.
  ...
  switch (method) {
    case 'initialize':
      return reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'charwork', version: VERSION },
      });
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (err instanceof ToolError) {
          return reply({ content: [{ type: 'text', text: `charwork: ${err.message}` }], isError: true });
        }
        throw err;
      }
      if (!result) {
